...
| `provider` | string | 否 | 提供商筛选：`outlook` / `imap` / `custom` |
...
- 对长期邮箱，若 `claim-random` 时显式传入非空 `project_key`，且回传时 `caller_id / task_id` 与领取记录一致，则：
  ...
  - 成功后可被其他项目立即再次领取
- 未传 `project_key`、`project_key` 为空串时，`success` 继续沿用旧语义并返回 `used`
...
能，但仅限长期邮箱项目复用路径：领取时显式传入非空 `project_key`，并保持 `caller_id / task_id` 一致时，`success` 后账号会回到 `available`，同项目被 success 记录拦截、跨项目可立即复用。未传 `project_key` 仍按旧语义进入 `used`。
